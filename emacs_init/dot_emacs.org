...
#+begin_src emacs-lisp
  (use-package consult
    ...
    ;; Exclude virtual environments and build artifact directories from grep.
    ;; This affects Python (.venv, .tox, __pycache__) and other ecosystems
    ;; (node_modules, dist, build). consult-ripgrep is an alternative that
    ;; achieves the same result automatically via .gitignore. It needs rg
    ;; installed in the system. I chose the latter, and thus commented this.
    ;; (setq consult-grep-args
    ;;       '("grep" "--null" "--line-buffered" "--color=never" "--no-heading"
    ;;         "--with-filename" "--line-number" "--recursive" "--extended-regexp"
    ;;         "--exclude-dir=.git"
    ;;         "--exclude-dir=.tox"
    ;;         "--exclude-dir=.venv"
    ;;         "--exclude-dir=.env"
...
Optional: sidebar with symbols and project browser
