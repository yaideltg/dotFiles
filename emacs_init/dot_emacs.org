...
* Programming IDE
:PROPERTIES:
:ID:       80211d26-d181-4b24-bb65-64cac814f1e1
:END:

Core LSP functionality (works for all languages). At work I had to create a Python virtual environment at =~/.venvs/pyright-venv= so I can install language servers, and then point Emacs to use it. Thus, in every configuration that the =venv= is specified, it's recovering the =lsp= language server from that =venv=. Consequently, for new language servers, they need to be installed in that =venv=, and then configured in the =use-package= section, as done for some cases down below. Also, in the case of ~bash-language-server~ it needs to be installed with =npm= and not in the =venv=. But Emacs is not usually able to find it because it normally doesn't load the full =PATH=. A solution would be to check where was it installed with ~which bash-language-server~ and then add it to the execution path ~(add-to-list 'exec-path "/home/youruser/.npm-global/bin")~. However, there's a better way: using package ~exec-path-from-shell~. We start by setting that, and the continue to =lsp=.
...
#+begin_src emacs-lisp
  (use-package lsp-mode
    :ensure t
    :hook ((prog-mode . (lambda ()  ;; start LSP for any programming mode but these
                        (unless (derived-mode-p 'apptainer-mode)
                          (lsp-deferred)))))
    ...
    ;; 1) prevent LSP from being triggered for apptainer-mode
    ;; 2) tells LSP that when it's in a scheme-mode buffer, for example, it should identify
    ;;    the language as "scheme" to the server. This avoids warnings about not knowing
...
#+begin_src emacs-lisp
  (use-package lsp-pyright
    ...
    :after lsp-mode
    :config
    (add-to-list 'lsp-client-packages 'lsp-pyright)
...
Following on the comments on =venv= for the =lsp= servers at the beginning of [[id:80211d26-d181-4b24-bb65-64cac814f1e1][this section]], in this specific case I needed to create another =venv= (which I realized is possible, thus we could create a =venv= for each language server to avoid conflicts). I couldn't install the =cmake-language-server= in the same as =pyright= because it required specific versions of a package, and then I didn't wanted a conflict. That dependency with a specific version required is =pygls=. Long story short, install this ~uv pip install cmake-language-server "pygls>=1.1.1,<2.0" --force-reinstall~ on the =cmake= =venv=, and it should work. test it with =~/.venvs/cmake-venv/bin/cmake-language-server --version=.

#+begin_src emacs-lisp
  (use-package cmake-mode
    :ensure t
    :hook (cmake-mode . lsp-deferred) ;; Redundant due to (prog-mode . lsp-deferred)
    ;; but it makes the intent clear and avoids any potential loading-order surprises
    :config
    (with-eval-after-load 'lsp-cmake
      (setq lsp-cmake-server-command
          (expand-file-name "~/.venvs/cmake-venv/bin/cmake-language-server"))))
#+end_src

** Makefile

#+begin_src emacs-lisp
  (add-to-list 'exec-path (expand-file-name "~/.venvs/makefile-venv/bin"))

  (with-eval-after-load 'lsp-makefile
    ;; (setq lsp-makefile-server-command
          ;; (list (expand-file-name "~/.venvs/makefile-venv/bin/make-language-server")))
    (setq lsp-makefile-autotools-server-command
          (list (expand-file-name "~/.venvs/makefile-venv/bin/autotools-language-server"))))
#+end_src
...
For coding in Guile Scheme, these two packages are quite helpful in REPL, evaluation, completion, documentation lookup, and more.
...
Additionally, LSP complaint about not being able to access the server when loading Guile Scheme files, thus I installed it in a separate =venv= and set it up here. That said, probably =geiser= and =geiser-guile= are all we need for guile development, and actually =semgrep= support for guile seems to be in early stages. Anyway, I included it here, it shouldn't be counterproductive.

#+begin_src emacs-lisp
  (with-eval-after-load 'lsp-semgrep
    (setq lsp-semgrep-server-command
          '("~/.venvs/semgrep/bin/semgrep" "lsp")))
#+end_src

If at some point we would like to deactivate LSP support for Guile Scheme, something like the following would be activated to disable it, while commenting the above snippet:

#+begin_src emacs-lisp
  ;; Disable semgrep-ls for Scheme files (not installed, not needed)
  ;; (add-hook 'scheme-mode-hook
            ;; (lambda ()
              ;; (setq-local lsp-disabled-clients '(semgrep-ls))))
#+end_src

** Colorizing delimiters

This colorizes the delimiters that surround point (the current =sexp= and all its ancestors), and updates live as you move the cursor. Sibling expressions stay uncolored.
...
#+begin_src emacs-lisp
  (use-package highlight-parentheses
    ...
    (setq highlight-parentheses-colors
          '("red" "orange" "yellow" "green" "cyan" "violet" "magenta")))
#+end_src

This colorizes all delimiters in the buffer, and isn't updated based on the pointer's position:
...
This is the Emacs - GNU Guix integration. It let's you manage guix package management from within Emacs. Similarly to what Magit is to Git. It's only useful when you use Guix as the daily driver package manager. 

#+begin_src emacs-lisp
  (use-package guix
    :ensure t
    :config
    ;; Semgrep LSP (static analysis for Scheme and other languages)
    (with-eval-after-load 'lsp-semgrep
      (setq lsp-semgrep-server-command
            '("~/.venvs/semgrep/bin/semgrep" "lsp")))
    )
